...
class OpenBBEquityPriceFetcher(BaseFetcher):
    """Fetcher for equity price bars via OpenBB."""
    ...
    def transform(self, raw_df: pd.DataFrame) -> pd.DataFrame:
        ...
        normalized = openbb_client.normalize_ohlcv(
            raw_df,
            symbol=self.symbol,
        ...
        return pd.DataFrame(
            {
                "provider": self.provider or "openbb",
                "provider_symbol": normalized["symbol"],
                "security_id": "openbb:" + normalized["symbol"].astype(str),
                "listing_id": "openbb:" + normalized["symbol"].astype(str),
                ...
                "currency": None,
                "adjustment_status": "adjusted_preferred" if self.prefer_adjusted else "raw_close",
                "source_file": None,
                "ingested_at_utc": now,
            }
        )
...
def main() -> int:
    ...
    logger.info(
        "Equity price ingest finished: %s succeeded, %s failed.",
        succeeded,
        len(failed_symbols),
    )
    if failed_symbols:
